...
/**
 * The different ways that {@link Job}s can manage how they execute {@link Task}
 * s.
 * 
 * @author Eric F. Savage <code@efsavage.com>
 * 
 */
public enum ExecutionStrategy implements IdentifiableEnum<String> {

	/**
	 * SERIAL_ABORT.
	 */
	SERIAL_ABORT("1", "srf", "Serial (abort on error)", "If a task fails, abort the job.", false, false, false),
	/**
	 * SERIAL_CONTINUE.
	 */
	SERIAL_CONTINUE("2", "src", "Serial (continue on error)", "If a task fails, continue the job.", false, false, false);
	 ...
	 */
	public static ExecutionStrategy get(final String string) {
		for (final ExecutionStrategy type : values()) {
	...
	private ExecutionStrategy(final String id, final String code, final String name, final String description, final boolean active, final boolean error, final boolean deleted) {
		this.id = id;
		this.code = code;
		this.name = name;
