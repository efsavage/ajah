...
public class EmailVerificationManager {
	 ...
	 */
	public DataOperationResult<EmailVerification> save(EmailVerification emailVerification) throws DataOperationException {
		...
		if (create) {
			DataOperationResult<EmailVerification> result = this.emailVerificationDao.insert(emailVerification);
			log.fine("Created EmailVerification " + emailVerification.getCode() + " [" + emailVerification.getId() + "]");
			return result;
		}
		DataOperationResult<EmailVerification> result = this.emailVerificationDao.update(emailVerification);
	 ...
	 */
	public EmailVerification load(EmailVerificationId emailVerificationId) throws DataOperationException, EmailVerificationNotFoundException {
		EmailVerification emailVerification = this.emailVerificationDao.load(emailVerificationId);
	 ...
	 *             If the query could not be executed.
	 */
	public DataOperationResult<EmailVerification> create(UserId userId, EmailId emailId) throws DataOperationException {
		...
		DataOperationResult<EmailVerification> result = save(emailVerification);
		return result;
	}
	 ...
	 *             If the ID specified did not match any emailVerifications.
	 */
	public DataOperationResult<EmailVerification> error(EmailVerificationId emailVerificationId) throws DataOperationException, EmailVerificationNotFoundException {
		EmailVerification emailVerification = load(emailVerificationId);
	...
	public boolean verify(final UserId userId, final String code) throws EmailVerificationNotFoundException, DataOperationException, EmailNotFoundException, PermissionDeniedException {
		EmailVerification emailVerification = this.emailVerificationDao.find(code);
		if (emailVerification == null) {
			throw new EmailVerificationNotFoundException(code);
		}
		Email email = this.emailManager.load(emailVerification.getEmailId());
