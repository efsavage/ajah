...
/**
 * Channels by which a reset request can be sent.
 *
 * @author Eric F. Savage <code@efsavage.com>
 *
 */
public enum ResetPasswordRequestChannel implements IdentifiableEnum<String> {
	 ...
	 * SMS Message.
	 */
	SMS("2", "sms", "SMS", "SMS/Text Message.");

	/**
	 * Finds a ResetPasswordRequestChannel that matches the id on id, name, or
	 * name().
	 *
	 * @param id
	 *            Value to match against id, name, or name()
	 * @return Matching ResetPasswordRequestChannel, or null.
	 */
	public static ResetPasswordRequestChannel get(final String id) {
		for (final ResetPasswordRequestChannel type : values()) {
	...
	private ResetPasswordRequestChannel(final String id, final String code, final String name, final String description) {
		this.id = id;
		this.code = code;
		this.name = name;
