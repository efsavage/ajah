...
/**
 * Valid states of UserAudit entities.
 * 
 * @author Eric F. Savage <code@efsavage.com>
 * 
 */
public enum UserAuditType implements IdentifiableEnum<String> {

	/**
	 * The user changed their own value.
	 */
	USER("0", "user", "User", "The user changed their own value.."),
	/**
	 * An admin changed the value.
	 */
	ADMIN("1", "admin", "Admin", "An admin changed the value.");

	/**
	 * Finds a UserAuditType that matches the id on id, name, or name().
	 * 
	 * @param string
	 *            Value to match against id, name, or name()
	 * @return Matching UserAuditType, or null.
	 */
	public static UserAuditType get(final String string) {
		for (final UserAuditType type : values()) {
	...
	private UserAuditType(final String id, final String code, final String name, final String description) {
		this.id = id;
		this.code = code;
		this.name = name;
